<footer>
      <div class="footer-container">
        <div class="footer-info">
          <p>📞 +34 972 98 10 20 | ✉️ gti@gtiempresas.com</p>
          <p>📍 C/ Nocedemora 20, Oviedo</p>
          <p><a href= "../GTI/LandingPage_No_Registrado.php#contacto">Contacto</a> | <a href="../GTI/PROA_No registrado.php">PROA</a></p>
        </div>
        <div class="footer-logo-block">
          <a href="../GTI/LandingPage_No_Registrado.php" class="logo">
            <span class="g">g</span><span class="ti">ti</span>
          </a>
          <p class="footer-copy">&copy; 2025 GTI. Todos los derechos reservados.</p>
        </div>
      </div>
    </footer>